package com.example.sipentas.models.verifikasi_atensi

data class VerifikasiAtensiList(
    val id_jenis_atensi:String? = null,
    val jenis:String? = null,
    val nilai:String? = null,
    val nama_jenis_atensi:String? = null,
    val nama_pm:String? = null,
    val id_atensi:Int? = null,
    val id_pm:String? = null,
    val id_assessment:String? = null,
    val petugas_assesmen:String? = null,
    val tanggal_assesmen:String? = null,
    val id_pendekatan_atensi:Int? = null,
    val nama_pendekatan_atensi:String? = null,
    val lat:String? = null,
    val long:String? = null,
    val nik_pm:String? = null,
    val tanggal:String? = null,
    val penerima:String? = null,
    val foto:String? = null
)
